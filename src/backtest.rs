...
/// Build H1 forward returns: ln(close_{t+1} / close_t) per (symbol, timestamp).
fn build_forward_returns(rows: &[ExecRow]) -> HashMap<(i64, String), f64> {
    let mut by_symbol: HashMap<&str, Vec<(i64, f64)>> = HashMap::new();
    for r in rows {
        by_symbol.entry(&r.symbol).or_default().push((r.timestamp, r.close));
    }
    ...
    for (sym, bars) in &by_symbol {
        ...
        for i in 0..sorted.len().saturating_sub(1) {
            let (ts, close) = sorted[i];
            let next_close = sorted[i + 1].1;
            if close > 0.0 && next_close.is_finite() {
                fwd.insert((ts, sym.to_string()), next_close / close - 1.0);
            }
        }
    }
    fwd
...
/// Build EWMA ADV lookup from execution rows.
fn build_adv_lookup_from_rows(rows: &[ExecRow], halflife: usize) -> HashMap<(i64, String), f64> {
    let mut by_symbol: HashMap<&str, Vec<(i64, f64, f64)>> = HashMap::new();
    for r in rows {
        by_symbol.entry(&r.symbol).or_default().push((r.timestamp, r.volume, r.close));
    }
    ...
    for (sym, bars) in &by_symbol {
        ...
        let volumes: Vec<f64> = sorted.iter().map(|(_, v, _)| *v).collect();
        let closes: Vec<f64> = sorted.iter().map(|(_, _, c)| *c).collect();
        let adv = cost::ewma_adv(&volumes, &closes, halflife);
        for (i, (ts, _, _)) in sorted.iter().enumerate() {
            lookup.insert((*ts, sym.to_string()), adv[i]);
        }
    }
    lookup
}

/// Build EWMA ADV in USD from execution rows.
/// Volume in the execution parquet is already in USD — no close multiplication needed.
fn build_adv_usd_lookup(rows: &[ExecRow], halflife: usize) -> HashMap<(i64, String), f64> {
    let alpha = 1.0 - (-2.0_f64.ln() / halflife as f64).exp();

    let mut by_symbol: HashMap<&str, Vec<(i64, f64)>> = HashMap::new();
    for r in rows {
        by_symbol.entry(&r.symbol).or_default().push((r.timestamp, r.volume));
    }
    ...
    for (sym, bars) in &by_symbol {
        ...
        let mut ewma = sorted[0].1;
        lookup.insert((sorted[0].0, sym.to_string()), ewma);
        for i in 1..sorted.len() {
            let vol = sorted[i].1;
            if vol.is_finite() {
                ewma = alpha * vol + (1.0 - alpha) * ewma;
            }
            lookup.insert((sorted[i].0, sym.to_string()), ewma);
        }
    }
    lookup
}

/// Build EWMA realized vol lookup (daily log-return std) from execution rows.
fn build_sigma_lookup_from_rows(rows: &[ExecRow], halflife: usize) -> HashMap<(i64, String), f64> {
    let alpha = 1.0 - (-2.0_f64.ln() / halflife as f64).exp();
    ...
    for (sym, bars) in &by_symbol {
        ...
        for i in 0..sorted.len() {
            // Sigma for this bar uses state from PREVIOUS bars (no lookahead)
            let sigma = if n_seen >= 2 {
                let var = (ewma_sq - ewma_mean * ewma_mean).max(0.0);
                var.sqrt()
            } else {
                0.0
            };
            lookup.insert((sorted[i].0, sym.to_string()), sigma);

            // Update EWMA with this bar's log return
            if i > 0 {
                let prev_close = sorted[i - 1].1;
                let close = sorted[i].1;
...
// ---------------------------------------------------------------------------
// Interned-key variants of lookup builders
// ---------------------------------------------------------------------------

fn build_forward_returns_interned(rows: &[ExecRow], symbols: &SymbolTable) -> HashMap<(i64, u32), f64> {
    let mut by_sym: HashMap<u32, Vec<(i64, f64)>> = HashMap::new();
    for r in rows {
        let sid = symbols.to_id[&r.symbol];
        by_sym.entry(sid).or_default().push((r.timestamp, r.close));
    }
    let mut fwd = HashMap::with_capacity(rows.len());
    for (sid, bars) in &by_sym {
        let mut sorted = bars.clone();
        sorted.sort_by_key(|(ts, _)| *ts);
        for i in 0..sorted.len().saturating_sub(1) {
            let (ts, close) = sorted[i];
            let next_close = sorted[i + 1].1;
            if close > 0.0 && next_close.is_finite() {
                fwd.insert((ts, *sid), next_close / close - 1.0);
            }
        }
    }
    fwd
...
fn build_sigma_lookup_interned(rows: &[ExecRow], halflife: usize, symbols: &SymbolTable) -> HashMap<(i64, u32), f64> {
    let alpha = 1.0 - (-2.0_f64.ln() / halflife as f64).exp();
    let mut by_sym: HashMap<u32, Vec<(i64, f64)>> = HashMap::new();
    for r in rows {
        by_sym.entry(symbols.to_id[&r.symbol]).or_default().push((r.timestamp, r.close));
    }
    ...
    for (sid, bars) in &by_sym {
        ...
        for i in 0..sorted.len() {
            ...
            lookup.insert((sorted[i].0, *sid), sigma);
            if i > 0 {
                let prev_close = sorted[i - 1].1;
                let close = sorted[i].1;
                if prev_close > 0.0 && close > 0.0 {
                    let r = (close / prev_close).ln();
                    if r.is_finite() {
                        if n_seen == 0 { ewma_mean = r; ewma_sq = r * r; }
                        else { ewma_mean = alpha * r + (1.0 - alpha) * ewma_mean; ewma_sq = alpha * r * r + (1.0 - alpha) * ewma_sq; }
                        n_seen += 1;
                    }
                }
            }
...
fn build_adv_usd_lookup_interned(rows: &[ExecRow], halflife: usize, symbols: &SymbolTable) -> HashMap<(i64, u32), f64> {
    let alpha = 1.0 - (-2.0_f64.ln() / halflife as f64).exp();
    let mut by_sym: HashMap<u32, Vec<(i64, f64)>> = HashMap::new();
    for r in rows {
        by_sym.entry(symbols.to_id[&r.symbol]).or_default().push((r.timestamp, r.volume));
    }
    let mut lookup = HashMap::with_capacity(rows.len());
    for (sid, bars) in &by_sym {
        let mut sorted = bars.clone();
        sorted.sort_by_key(|(ts, _)| *ts);
        // ADV(t) must use only volumes from bars STRICTLY BEFORE t.
        // Bar 0 has no past — seed with bar 0's vol (warmup; simulation skips
        // these bars via has_qp_solution gate before any decisions are made).
        let mut ewma = sorted[0].1;
        lookup.insert((sorted[0].0, *sid), ewma);
        for i in 1..sorted.len() {
            // Insert BEFORE updating: ADV at bar i reflects vols 0..i-1 only.
            lookup.insert((sorted[i].0, *sid), ewma);
            let vol = sorted[i].1;
            if vol.is_finite() { ewma = alpha * vol + (1.0 - alpha) * ewma; }
        }
    }
    lookup
}

/// Compute per-bar cross-sectional Pearson IC between alpha and forward returns.
fn compute_per_bar_ic(
    alphas: &[AlphaRow],
    forward_returns: &HashMap<(i64, String), f64>,
) -> Vec<(i64, f64)> {
    let mut by_ts: HashMap<i64, Vec<(f64, f64)>> = HashMap::new();
    for row in alphas {
        if let Some(&ret) = forward_returns.get(&(row.timestamp, row.symbol.clone())) {
            if ret.is_finite() && row.alpha_future.is_finite() {
                by_ts.entry(row.timestamp).or_default().push((row.alpha_future, ret));
            }
        }
    }

    let mut result: Vec<(i64, f64)> = Vec::new();
    for (ts, pairs) in &by_ts {
        let n = pairs.len();
        if n < 5 { continue; }
        let mean_a = pairs.iter().map(|(a, _)| a).sum::<f64>() / n as f64;
        let mean_r = pairs.iter().map(|(_, r)| r).sum::<f64>() / n as f64;
        let mut cov = 0.0;
        let mut var_a = 0.0;
        let mut var_r = 0.0;
        for (a, r) in pairs {
        ...
        let denom = (var_a * var_r).sqrt();
        if denom > 1e-12 {
            result.push((*ts, cov / denom));
        }
    }
...
/// Shadow portfolio replay: run the final weights through the PortfolioBook
/// to verify NAV tracking, weight drift, and PnL decomposition.
///
/// This is a diagnostic tool — it doesn't change the pipeline output.
/// It replays the same weight decisions through position state to show
/// how the portfolio ACTUALLY evolves with market moves.
///
/// Uses ExchangeConfig for fee computation (replacing hardcoded values)
/// and tracks margin utilization.
fn shadow_portfolio_replay(
    weights: &[WeightOutput],
    rows: &[ExecRow],
    config: &ExecutionConfig,
) {
    use crate::exchange::VolumeTracker;
    use crate::position::PortfolioBook;

    let nav_start = config.nav_usd;
    let exchange = &config.exchange;
    let mut book = PortfolioBook::new(nav_start);
    let mut volume_tracker = VolumeTracker::new();

    // Build price lookup: (timestamp, symbol) -> close
    let prices: HashMap<(i64, String), f64> = rows
        .iter()
        .map(|r| ((r.timestamp, r.symbol.clone()), r.close))
        .collect();

    // Build funding lookup
    let funding: HashMap<(i64, String), f64> = rows
        .iter()
        .map(|r| ((r.timestamp, r.symbol.clone()), r.funding_rate))
        .collect();

    // Group weights by timestamp
    let mut by_ts: std::collections::BTreeMap<i64, Vec<&WeightOutput>> = std::collections::BTreeMap::new();
    for w in weights {
    ...
    let mut max_weight_drift = 0.0f64;
    let mut n_funding_capped = 0usize;
    let mut peak_margin_ratio = f64::INFINITY;
    let mut min_margin_ratio = f64::INFINITY;
    let mut n_liquidation_events = 0usize;

    for (&ts, bar_weights) in &by_ts {
        n_bars += 1;

        // 1. Mark to market with this bar's prices
        let bar_prices: HashMap<String, f64> = bar_weights
            .iter()
            .filter_map(|w| prices.get(&(ts, w.symbol.clone())).map(|&p| (w.symbol.clone(), p)))
            .collect();
        book.mark_all(&bar_prices);

        // 2. Accrue funding via exchange adapter
        for (sym, pos) in &mut book.positions {
            if let Some(&rate) = funding.get(&(ts, sym.clone())) {
                if let Some(&price) = bar_prices.get(sym) {
                    // Track cap hits: check if rate exceeds cap
                    if let Some(cap) = exchange.funding_cap_per_interval {
                        if rate.abs() > cap {
                            n_funding_capped += 1;
                        }
                    }
                    let cf = exchange.compute_funding_payment(pos.quantity, price, rate);
        ...
        // 3. Compute weight drift: how far actual weights are from target weights
        let actual_w = book.actual_weights();
        for w in bar_weights {
            let actual = actual_w.get(&w.symbol).copied().unwrap_or(0.0);
            let drift = (actual - w.weight_final).abs();
            if drift > max_weight_drift {
                max_weight_drift = drift;
            }
        }

        // 4. Execute trades to reach target weights
        let nav = book.nav();
        if nav > 1e-6 {
            for w in bar_weights {
                let target_notional = w.weight_final * nav;
                let current_notional = book.positions
                    .get(&w.symbol)
                    .map(|p| p.quantity * p.mark_price)
                    .unwrap_or(0.0);
                let delta_notional = target_notional - current_notional;

                if delta_notional.abs() < 1.0 {
                    continue; // skip dust trades
                }

                let price = bar_prices.get(&w.symbol).copied().unwrap_or(0.0);
                if price <= 0.0 { continue; }

                let delta_qty = delta_notional / price;
                let trailing_vol = volume_tracker.trailing_volume();
                let commission = exchange.compute_fee(delta_notional, false, trailing_vol);

                let pos = book.get_or_create_position(&w.symbol, ts);
                let rpnl = pos.apply_fill(delta_qty, price, commission, ts);
                book.cash += rpnl;
                book.cash -= commission;
                total_commission += commission;

                // Track volume for fee tier progression
                volume_tracker.record_trade(ts, delta_notional);
            }
        }

        // 5. Margin check
        let total_maintenance: f64 = book.positions.values()
            .map(|p| exchange.maintenance_margin(p.notional()))
            .sum();
        let current_nav = book.nav();
        let mr = exchange.margin_ratio(current_nav, total_maintenance);
        if mr < min_margin_ratio { min_margin_ratio = mr; }
        if mr < peak_margin_ratio { peak_margin_ratio = mr; }
        if exchange.is_liquidatable(current_nav, total_maintenance) {
            n_liquidation_events += 1;
        }

        // Clean up flat positions periodically
        if n_bars % 100 == 0 {
    ...
    // Final margin state
    let final_maintenance: f64 = book.positions.values()
        .map(|p| exchange.maintenance_margin(p.notional()))
        .sum();
    let final_nav = book.nav();
    let final_margin_ratio = exchange.margin_ratio(final_nav, final_maintenance);

    let total_return = (final_nav / nav_start - 1.0) * 100.0;
    let trading_pnl = book.total_realized_trading_pnl() + book.total_unrealized_pnl();

    // PnL consistency check: NAV change should equal trading + funding - commission
    let nav_change = final_nav - nav_start;
    let pnl_sum = trading_pnl + total_funding - total_commission;
    let pnl_discrepancy = (nav_change - pnl_sum).abs();

    // Final fee tier info
    let final_trailing_vol = volume_tracker.trailing_volume();
    let (final_taker_bps, final_maker_bps) = exchange.fee_bps_for_volume(final_trailing_vol);
    let (start_taker_bps, _) = exchange.fee_bps_for_volume(0.0);

    println!("\n  === Shadow Portfolio Book ===");
    println!("  Exchange:         {} (tier 0: {:.1}bps taker)",
        exchange.name, start_taker_bps);
    println!("  Start NAV:        ${:.2}", nav_start);
    println!("  Final NAV (book): ${:.2}", final_nav);
    println!("  Total Return:     {:.1}%", total_return);
    println!("  ---");
    println!("  Trading PnL:      ${:.2}", trading_pnl);
    println!("  Funding PnL:      ${:.2}", total_funding);
    println!("  Commission:       ${:.2}", total_commission);
    println!("  NAV change:       ${:.2}", nav_change);
    println!("  PnL check:        ${:.2} (trading+funding-commission)", pnl_sum);
    println!("  PnL discrepancy:  ${:.4}", pnl_discrepancy);
    println!("  ---");
    println!("  Cash balance:     ${:.2}", book.cash);
    println!("  Open positions:   {} ({} L / {} S)", book.positions.len(), book.n_long(), book.n_short());
    println!("  Gross exposure:   ${:.2} ({:.1}x NAV)", book.gross_exposure(), book.gross_leverage());
    println!("  Max weight drift: {:.4}", max_weight_drift);
    println!("  Bars processed:   {}", n_bars);
    println!("  --- Exchange Diagnostics ---");
    println!("  14d trailing vol: ${:.0} → {:.1}bps taker / {:.1}bps maker",
        final_trailing_vol, final_taker_bps, final_maker_bps);
    println!("  Funding cap hits: {} ({:.1}% of position-bars)",
        n_funding_capped,
        if n_bars > 0 { n_funding_capped as f64 / n_bars as f64 * 100.0 } else { 0.0 });
    println!("  Min margin ratio: {:.2}x", min_margin_ratio);
    println!("  Final margin:     {:.2}x (maintenance ${:.2})", final_margin_ratio, final_maintenance);
    if n_liquidation_events > 0 {
        println!("  LIQUIDATIONS:     {} bars below maintenance", n_liquidation_events);
    }

    if pnl_discrepancy > 1.0 {
        println!("  ⚠ PnL DISCREPANCY: NAV change doesn't match PnL components by ${:.2}", pnl_discrepancy);
        println!("    This could indicate a bug in position accounting or cash flow tracking.");
    }
}
